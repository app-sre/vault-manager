...
oc wait --for=condition=Ready pod -l app=keycloak --timeout=10m || {
  ...
  oc logs -l app=keycloak --tail=20
  exit 1
}
...
oc wait --for=condition=Complete job/vault-init --timeout=1m
...
echo "  oc port-forward svc/keycloak 8180:8180"
echo "  oc port-forward svc/qontract-server 4000:4000"
echo "  oc port-forward svc/primary-vault 8200:8200"
echo "  oc port-forward svc/secondary-vault 8202:8202"
echo ""
echo "To run tests:"
echo "  oc apply -f ${SCRIPT_DIR}/test-runner-pod.yaml"
echo "  oc logs -f vault-manager-test"
