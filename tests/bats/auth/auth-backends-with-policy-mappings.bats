...
@test "test vault-manager manage auth backends and policy mappings" {
    ...
    [[ "${output}" == *"[Vault Auth] successfully enabled auth backend"*"instance=\"http://127.0.0.1:8200\""*"path=kubernetes-appsres03ue1/"*"type=kubernetes"* ]]
    ...
    [[ "${output}" == *"[Vault Auth] auth backend successfully configured"*"instance=\"http://127.0.0.1:8200\""*"path=auth/kubernetes-appsres03ue1/config"*"type=kubernetes"* ]]
